<x-app-layout>
    <x-slot:title>Admin Update Order</x-slot>
    ...
    <div class="flex justify-center items-center bg-white my-20">
        <div class="bg-white rounded-lg shadow-md p-8 shadow-black container mx-auto overflow-y-scroll">
            ...
            <form method="POST" action="{{ route('admin.order.update', ['id' => $order->id]) }}" class="mt-5">
                @csrf
                @method('put')

                <div class="mb-4">
                    <x-input-label for="order_status" :value="__('Order Status')" />
                    <select class="w-full px-4 py-2 rounded-md border border-gray-300" name="order_status" id="order_status" required>
                ...
                <button type="submit" class="w-full bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 transition-colors disabled:bg-blue-200">Update Order</button>
            </form>
        </div>
    </div>
</x-app-layout>